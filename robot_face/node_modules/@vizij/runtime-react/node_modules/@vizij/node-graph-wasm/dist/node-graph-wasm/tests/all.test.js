...
import { init, Graph, oscillatorBasics, vectorPlayground, tupleSpringDampSlew, layeredRigBlend, hierarchicalBlend, weightedAverage, nestedTelemetry, graphSamples, urdfIkPosition, loadNodeGraphSpec, loadNodeGraphSpecJson, getNodeRegistry, findNodeSignature, requireNodeSignature, listNodeTypeIds, groupNodeSignaturesByCategory, nodeRegistryVersion, } from "../src/index.js";
const here = dirname(fileURLToPath(import.meta.url));
function pkgWasmUrl() {
    const wasmPath = resolve(here, "../../../pkg/vizij_graph_wasm_bg.wasm");
    if (!existsSync(wasmPath)) {
        throw new Error("Missing pkg/vizij_graph_wasm_bg.wasm. Run:\n  wasm-pack build crates/node-graph/vizij-graph-wasm --target web --out-dir npm/@vizij/node-graph-wasm/pkg --release (from repo root vizij-rs/)");
    }
    return pathToFileURL(wasmPath);
}
async function loadJsonFixture(key) {
...
function ensureFiniteNumber(value, context) {
...
function assertValueFinite(value, context) {
    ...
    const obj = value;
    if ("float" in obj) {
        ensureFiniteNumber(obj.float, context);
        return;
    }
    if ("bool" in obj)
        return;
    if ("text" in obj)
        return;
    if ("vec2" in obj) {
        obj.vec2.forEach((v, idx) => ensureFiniteNumber(v, `${context}.vec2[${idx}]`));
        return;
    }
    if ("vec3" in obj) {
        obj.vec3.forEach((v, idx) => ensureFiniteNumber(v, `${context}.vec3[${idx}]`));
        return;
    }
    if ("vec4" in obj) {
        obj.vec4.forEach((v, idx) => ensureFiniteNumber(v, `${context}.vec4[${idx}]`));
        return;
    }
    if ("quat" in obj) {
        obj.quat.forEach((v, idx) => ensureFiniteNumber(v, `${context}.quat[${idx}]`));
        return;
    }
    if ("color" in obj) {
        obj.color.forEach((v, idx) => ensureFiniteNumber(v, `${context}.color[${idx}]`));
        return;
    }
    if ("transform" in obj) {
        const transform = obj.transform;
        transform.translation.forEach((v, idx) => ensureFiniteNumber(v, `${context}.transform.translation[${idx}]`));
        transform.rotation.forEach((v, idx) => ensureFiniteNumber(v, `${context}.transform.rotation[${idx}]`));
        transform.scale.forEach((v, idx) => ensureFiniteNumber(v, `${context}.transform.scale[${idx}]`));
        return;
    }
    if ("vector" in obj) {
        obj.vector.forEach((v, idx) => ensureFiniteNumber(v, `${context}.vector[${idx}]`));
        return;
    }
    if ("record" in obj) {
        for (const [key, child] of Object.entries(obj.record)) {
            assertValueFinite(child, `${context}.${key}`);
        }
        return;
    }
    if ("array" in obj) {
        obj.array.forEach((child, idx) => assertValueFinite(child, `${context}[${idx}]`));
        return;
    }
    if ("list" in obj) {
        obj.list.forEach((child, idx) => assertValueFinite(child, `${context}.list[${idx}]`));
        return;
    }
    if ("tuple" in obj) {
        obj.tuple.forEach((child, idx) => assertValueFinite(child, `${context}.tuple[${idx}]`));
        return;
    }
    if ("enum" in obj) {
...
const EPSILON = 1e-4;
function assertNearlyEqual(actual, expected, context) {
    if (Math.abs(actual - expected) > EPSILON) {
        throw new Error(`${context} expected ${expected} but received ${actual}`);
    }
}
function expectNumericVector(value, expected, label) {
    if (typeof value === "number") {
    ...
    let actual;
    if ("vector" in obj) {
        actual = obj.vector.slice();
    }
    else if ("vec2" in obj) {
        actual = obj.vec2.slice();
    }
    else if ("vec3" in obj) {
        actual = obj.vec3.slice();
    }
    else if ("vec4" in obj) {
        actual = obj.vec4.slice();
    }
    else if ("quat" in obj) {
        actual = obj.quat.slice();
    }
    if (!actual) {
...
function expectListOfText(value, expected, label) {
    if (!value || typeof value !== "object" || value === null || !("list" in value)) {
        throw new Error(`${label} expected list of text values`);
    }
    const listEntries = value.list;
    const actual = listEntries.map((entry, idx) => {
...
function expectTupleOfFloats(value, expected, label) {
...
function approxVector(actual, expected, eps = EPSILON) {
...
function valueToVector(value) {
    ...
    if ("vector" in obj)
        return obj.vector.slice();
    if ("vec4" in obj)
        return obj.vec4.slice();
    if ("vec3" in obj)
        return obj.vec3.slice();
    if ("vec2" in obj)
        return obj.vec2.slice();
    if ("quat" in obj)
        return obj.quat.slice();
    throw new Error(`Value is not a vector-like payload: ${JSON.stringify(value)}`);
}
...
async function runSample(name, spec, options = {}) {
    const g = new Graph();
    try {
        ...
        if (raw && typeof raw === "object" && Array.isArray(raw.nodes)) {
            for (let i = 0; i < raw.nodes.length; i += 1) {
                ...
                if (!node || typeof node !== "object" || typeof node.type !== "string") {
                    throw new Error(`Spec validation failed for sample '${name}': node at index ${i} missing or invalid 'type': ${JSON.stringify(node)}`);
                }
            }
        }
...
function requireWrite(result, path) {
...
function assertVector(write, expected, epsilon = 1e-4) {
...
function assertFloat(write, expected, epsilon = 1e-4) {
...
function assertBool(write, expected) {
...
function assertText(write, expected) {
    const obj = asValueObject(write.value);
...
(async () => {
    try {
        ...
        assert.equal(findNodeSignature("Constant"), constantSignature, "lookup should be case-insensitive");
        assert.equal(findNodeSignature("does-not-exist"), undefined, "lookup should return undefined for missing nodes");
        ...
        assert.equal(urdfSample, urdfIkPosition, "graphSamples should reference the urdfIkPosition sample");
        ...
        // stageInputs should route shaped payloads through path staging instead of scalar batch
        {
            ...
            graph.inner = {
                stage_inputs_batch: (paths, values) => {
                    batched.push({ paths: [...paths], values: Array.from(values) });
                },
            };
            graph.stageInputs(["samples/a", "samples/b"], new Float32Array([1.5, 2.5]), [null, { id: "Vec3" }]);
        ...
        // urdf-ik-position (shared fixture)
        {
            ...
            const record = solutionObj.record;
            ...
            jointNames.forEach((joint) => {
                ...
                assertNearlyEqual(entry.float, 0, joint);
            });
        }
        ...
        // URDF FK/IK round-trip coverage across multiple poses using shared fixture.
        {
            ...
            const expectedJointNames = ["joint1", "joint2", "joint3", "joint4", "joint5", "joint6"];
            fkIkRoundtripSamples.forEach((angles, sampleIdx) => {
                ...
                const fkNode = fkResult.nodes?.fk;
                assert.ok(fkNode, `fk node result missing for sample ${sampleIdx}`);
                const fkPositionValue = fkNode.position?.value;
                ...
                const ikNode = fkResult.nodes?.ik;
                assert.ok(ikNode, `ik node result missing for sample ${sampleIdx}`);
                const ikValue = ikNode.out?.value;
                ...
                expectedJointNames.forEach((jointName) => {
                    ...
                    const jointAngle = entryObj.float;
                ...
                const ikAngles = expectedJointNames.map((jointName) => {
                    ...
                    return typeof entryObj?.float === "number" ? entryObj.float : 0;
                });
                ...
                const validationFkNode = validationResult.nodes?.fk;
                const validationPosValue = validationFkNode?.position?.value;

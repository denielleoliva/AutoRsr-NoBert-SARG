...
import { toValueJSON, valueAsNumber, valueAsVector, valueAsQuat, valueAsTransform, valueAsText, } from "@vizij/value-json";
import { init, createOrchestrator, loadOrchestrationBundle } from "../src/index.js";
import { loadAnimationFixture as loadEmbeddedAnimation, loadNodeGraphSpec as loadEmbeddedGraphSpec, } from "../src/fixtures.js";
const here = dirname(fileURLToPath(import.meta.url));
function pkgWasmUrl() {
    const wasmPath = resolve(here, "../../../pkg/vizij_orchestrator_wasm_bg.wasm");
    if (!existsSync(wasmPath)) {
        throw new Error("Missing pkg/vizij_orchestrator_wasm_bg.wasm. Run:\n  npm run build:wasm:orchestrator (from repo root)");
    }
    return pathToFileURL(wasmPath);
}
function readScalar(writes, path) {
    const hit = writes.find((w) => w.path === path);
    assert.ok(hit, `Expected write for ${path}`);
    const value = valueAsNumber(hit.value);
...
function graphConfigFromFixture(spec) {
    if (spec && typeof spec === "object" && "spec" in spec) {
        return cloneDeep(spec);
    }
    return { spec: cloneDeep(spec) };
}
function graphConfigFromBinding(binding) {
...
const EPSILON = 1e-3;
function expectNearlyEqual(actual, expected, label) {
...
function expectValueMatches(actual, expected, label) {
    ...
    if ("vec2" in obj || "vec3" in obj || "vec4" in obj || "vector" in obj) {
        ...
        const expectedVec = (obj.vec2 || obj.vec3 || obj.vec4 || obj.vector);
    ...
    if ("record" in obj) {
        ...
        if (actualObj && "record" in actualObj) {
            const actualRecord = actualObj.record || {};
            const expectedRecord = obj.record;
            for (const [key, expectedValue] of Object.entries(expectedRecord)) {
                expectValueMatches(actualRecord[key], expectedValue, `${label}.${key}`);
            }
            return;
        }
...
function expectWriteMatches(writes, path, expected) {
    const hit = writes.find((w) => w.path === path);
    assert.ok(hit, `Expected write for ${path}`);
    expectValueMatches(hit.value, expected, path);
}
async function testScalarRampPipeline() {
    const bundle = await loadOrchestrationBundle("scalar-ramp-pipeline");
    const schedule = (typeof bundle.descriptor.schedule === "string" && bundle.descriptor.schedule) ||
        "SinglePass";
    ...
    const animationConfig = {
        ...
        setup: cloneDeep(primaryAnimation?.setup ?? { animation: primaryAnimation?.animation }),
    };
    ...
    for (const input of bundle.initialInputs ?? []) {
        ...
        orch.setInput(input.path, toValueJSON(input.value), shape);
    }
    for (const step of bundle.descriptor.steps ?? []) {
        ...
        for (const [path, expectedRaw] of Object.entries(step.expect ?? {})) {
            ...
            assert.ok(Math.abs(actual - expected) < 1e-3, `Expected ${path} ≈ ${expected}, received ${actual}`);
        }
    }
}
async function testChainedSlewPipeline() {
    ...
    const animationConfig = {
        setup: {
            ...
            player: { name: "chain-player", loop_mode: "once" },
        },
    };
    const animId = orch.registerAnimation(animationConfig);
    assert.ok(animId);
    ...
    const assertRateLimitedChange = (nextValue, prevValue, dtSeconds) => {
    ...
    steps.forEach((step, index) => {
        ...
        assert.ok(Math.abs(signValue - step.sign) < 1e-3, `expected sign ${step.sign}, got ${signValue}`);
        if (index > 0) {
            assertRateLimitedChange(slewValue, previousSlew, step.delta);
        }
        assert.ok(Math.abs(slewValue - step.slewed) < 1e-3, `expected slew ${step.slewed}, got ${slewValue}`);
        previousSlew = slewValue;
    });
}
async function testChainSignSlewFixture() {
    const bundle = await loadOrchestrationBundle("chain-sign-slew-pipeline");
    const schedule = (typeof bundle.descriptor.schedule === "string" && bundle.descriptor.schedule) ||
        "SinglePass";
    const orch = await createOrchestrator({ schedule });
    assert.ok(bundle.initialInputs?.some((input) => input.path === "chain/ramp.value"), "expected staged ramp value in initial inputs");
...
async function testMergedGraphRegistration() {
    const orch = await createOrchestrator({ schedule: "SinglePass" });
    let mergedId;
    try {
        ...
        if (message.includes("register_merged_graph is not a function")) {
            console.warn("Skipping merged graph test because wasm bindings were not rebuilt (run pnpm run build:wasm:orchestrator).");
            return;
        }
        throw err;
    }
...
async function testReplaceGraphStructuralEditResetsPlan() {
    const orch = await createOrchestrator({ schedule: "SinglePass" });
    ...
    const frame1 = orch.step(1 / 60);
    const writes1 = frame1.merged_writes ?? [];
    ...
    const frame2 = orch.step(1 / 60);
    const writes2 = frame2.merged_writes ?? [];
    expectNearlyEqual(readScalar(writes2, "shared/value"), 2, "post-replace value");
}
async function testBlendPosePipeline() {
    ...
    const animationConfig = {
        ...(primaryAnimation?.id ? { id: primaryAnimation.id } : {}),
        setup: cloneDeep(primaryAnimation?.setup ?? {
            animation: primaryAnimation?.animation,
            player: { name: "pose-player", loop_mode: "loop" },
        }),
    };
    try {
        orch.registerAnimation(animationConfig);
        ...
        const message = err instanceof Error ? err.message : String(err);
        const expected = message.includes("RawValue") || message.includes("stored animation parse error");
        assert.ok(expected, `unexpected animation registration failure: ${message}`);
        assert.ok(Array.isArray((primaryAnimation?.animation ?? {}).tracks), "blend pose animation tracks missing");
        return;
    }
    for (const input of bundle.initialInputs ?? []) {
        const shape = input.shape ? cloneDeep(input.shape) : undefined;
...
async function testMergedFixtureBundle() {
    ...
    const writes = frame.merged_writes ?? [];
    const expectScalar = (path, expected) => {
        const actual = readScalar(writes, path);
        expectNearlyEqual(actual, expected, path);
    };
